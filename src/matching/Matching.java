...
public abstract class Matching {
	 ...
	 * @return
	 */
	public boolean checkPartStudentAssaigned(Problem pro, int startPos,
			int endPos) {
		for (int i = startPos; i < endPos; i++) {
			if (!pro.getStudent()[pro.getMasterList()[i]].IsAssign()) {
				return false;
			}
		}
		return true;
	...
	public int getRequestFreeSeat(Problem pro) {
		...
		int rest = id.M_UPPERLIMIT_FREESEAT;
		int counter = 0;
